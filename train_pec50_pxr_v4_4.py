"""
pEC50 PXR — Pipeline v4_4
==========================
Base    : v4_3_3 (best single Chemprop, RAE=0.5581 on unblinded)

Changelog v4_3_3 -> v4_4
--------------------------
NEW  Training data: train_unimol_e3_ct2_crhi_semi.csv
       Replaces clean_train2.csv / train_v2_inactive300.csv.
       Adds 244 crude_nv_hi + 55 semi_nv compounds = 97 more high-actives
       at pEC50>=5.5 (467 vs 370), 41 more at pEC50>=6.0 (105 vs 64).

NEW  SC inactive supplement (--inactive_supplement_csv):
       Loads sc_inactives_300.csv (300 confirmed-inactive SC compounds,
       pseudo-pEC50=2.0) and appends to training data.  Mirrors the
       train_v2_inactive300 strategy that reduced inactive over-prediction.
       Combined dataset: 4342 rows (vs 4043 in v2_inact300).

CHANGED  High-tail defaults: mult=3.0, thresh=5.5 (was 1.0 / 6.0).
       Best Chemprop result (v2_ht5p5_3x, RAE=0.5581) used these values.
       The new base has 26%% more high-active compounds to amplify.

KEPT  All v4_3_3 architecture, MSE loss, activity-scaffold CV, 10-fold.

NEW      --no_sample_weights flag (default: weights ON).
           Allows a quick A/B test: run once with weights (default) and once
           with --no_sample_weights to see which actually helps on the
           blind test set.  OOF MAE may not reflect the difference because
           the weighting changes which compounds matter most.

Versioning
----------
  v4_1   — rank 26, MSE, tail 5x, no weight fix
  v4_3_1 — rank ~50, MSE, tail 5x, weight BUG (weights silently dropped)
  v4_3_2 — rank 59, Huber delta=0.5, tail 2x, weight fix  ← WORSE
  v4_3_3 — this file: MSE, tail 5x, weight fix, --no_sample_weights flag

Usage (Windows CMD)
-------------------
  python train_pec50_pxr_v4_3_3.py --mode BOTH

  :: Disable sample weights to A/B test
  python train_pec50_pxr_v4_3_3.py --mode BOTH --no_sample_weights ^
      --model_dir models_pec50_pxr_v4_3_3_nw ^
      --output    predictions_pec50_pxr_v4_3_3_nw.csv
"""
...
MODEL_SAVE_DIR = "models_pxr_v4_4"
OUTPUT_CSV     = "predictions_pxr_v4_4.csv"
...
class PXRMPNNPredictor:
    ...
    def _build(self, cfg, input_extra=0):
        ...
        # MSE loss — chemprop default; no custom criterion needed
        ffn = RegressionFFN(input_dim=hd + input_extra, hidden_dim=hd,
                            n_layers=nl, dropout=do, n_tasks=1)
        return MPNN(message_passing=mp, agg=MeanAggregation(),
                    predictor=ffn, batch_norm=True)

    def _safe_load(self, path):
        for kw in [{'weights_only': False}, {}]:
            try:
                return MPNN.load_from_checkpoint(
                    path, map_location=DEVICE, **kw)
            except Exception:
                continue
        return None
    ...
    def train(self, smiles_list, y_primary,
        ...
        print(f"\n{'='*60}")
        print(f"Training PXR MPNN v4_3_3  |  N={len(smiles_list)}  "
        ...
        for fold_idx, val_indices in enumerate(folds):
            ...
            if os.path.exists(ckpt_path):
                ...
                best = self._safe_load(ckpt_path)
                if best is None:
            ...
            if not ckpt_cb.best_model_path or \
               not os.path.exists(ckpt_cb.best_model_path):
                ...
                continue
            best = self._safe_load(ckpt_cb.best_model_path)
            if best is None:
                print(f"  WARNING: load failed for fold {fold_idx}")
                continue
            best.eval()
...
def main():
    parser = argparse.ArgumentParser(description="PXR pEC50 MPNN v4_3_3")
