"""
pEC50 PXR -- Pipeline v4_13f
==============================
Base    : v4_13d (multi-task MPNN, asymmetric classification head)

Changelog v4_13d -> v4_13f
---------------------------
CHANGED  Steeper tail upweighting to fight regression-to-the-mean.

  v4_13d used:  weight = clip(1 + |z|,       1, 5x)   -- linear in z
  v4_13f uses:  weight = clip(1 + 2*|z|^1.5, 1, 10x)  -- steeper growth

  Effect on gradient signal (vs v4_13d):
    pEC50=1.5  (z=2.7):  3.7x -> 10.0x  (+2.7x more gradient)
    pEC50=2.0  (z=2.3):  3.3x ->  7.8x  (+2.4x)
    pEC50=6.0  (z=1.4):  2.4x ->  4.5x  (+1.8x)
    pEC50=6.5  (z=1.9):  2.9x ->  6.3x  (+2.2x)
    pEC50=4.5  (z=0.0):  1.0x ->  1.0x  (unchanged)

  Rationale: regression-to-the-mean is driven by the dense 4-5.5 region
  dominating gradients. Upweighting the tails 2-3x more forces the model to
  take inactive (<3.5) and potent (>6) compounds more seriously. The existing
  5x cap was never being hit; the new 10x cap allows the formula to operate.

KEPT  All v4_13d architecture: asymmetric BCE, scaffold CV, hyperparameters.

Usage (Windows CMD)
-------------------
  python train_pec50_pxr_v4_13f.py --mode BOTH
"""
...
MODEL_SAVE_DIR = "models_pxr_v4_13f"
OUTPUT_CSV     = "predictions_pxr_v4_13f.csv"
...
def compute_sample_weights(df, min_w=0.1, max_w=5.0):
    ...
    if TARGET_COL in df.columns:
        ...
        z_scores = np.abs(y_vals - y_mean) / y_std
        tail_w = np.clip(
            1.0 + 2.0 * (z_scores ** 1.5),   # v4_13f: steeper |z|^1.5 formula (10x cap)
            1.0, TAIL_WEIGHT_MAX)
        n_tail = (z_scores > 1).sum()
...
class MultitaskMPNN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Regression loss: weighted MSE (all compounds)
        reg_loss = (w * (reg_pred - y_sc) ** 2).mean()

        # Asymmetric classification loss -- FALSE POSITIVE PENALTY ONLY.
        # Only inactive compounds (pEC50 < act_thresh) contribute to clf_loss.
        # Active compounds generate zero classification gradient, so genuine
        # actives are never pulled toward the inactive zone.
        inactive_mask = (y_sc <= self.act_thresh)
        if inactive_mask.any():
            # Target = 0 (inactive) for all compounds in this subset.
            # Loss fires when clf_logit > 0 (model thinks "active") for an
            # inactive compound -- i.e., a false positive.
            fp_logits  = clf_logit[inactive_mask]
            fp_targets = torch.zeros_like(fp_logits)
            fp_weights = w[inactive_mask]
            clf_loss = F.binary_cross_entropy_with_logits(
                fp_logits, fp_targets, weight=fp_weights)
        else:
            clf_loss = torch.tensor(0.0, device=y_sc.device,
                                    requires_grad=False)
...
class PXRMPNNPredictor:
    ...
    def train(self, smiles_list, y_primary,
        ...
        print(f"\n{'='*60}")
        print(f"Training PXR MPNN v4_13f |  N={len(smiles_list)}  "
        ...
        if patience_override is not None:
            ...
            print(f"Patience override: {patience_override}")
        ...
        for fold_idx, val_indices in enumerate(folds):
            ...
            ckpt_path = os.path.join(self.model_dir, f'{ckpt_fn}.ckpt')

            # ── Resume: skip training if checkpoint already saved ──────────
            if os.path.exists(ckpt_path):
                print(f"  Checkpoint found — skipping training (fold {fold_idx+1})")
                self.best_epochs.append(-1)  # epoch unknown for resumed fold
                best = self._load_from_ckpt(ckpt_path)
                if best is None:
                ...
                print(f"  [resumed] RMSE={rmse:.4f}  MAE={mae:.4f}  R2={r2:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="PXR pEC50 MPNN v4_13d")
    ...
    parser.add_argument('--patience', type=int, default=None,
                        help='Early stopping patience (epochs). Overrides the value in '
                             'ADAPTIVE_CONFIG for the selected data-size tier. '
                             'Default: use tier value (20 or 25).')
    ...
    if args.mode in ('TRAIN', 'BOTH'):
        ...
        # v4_13f: z-score tail loss -- applied unconditionally on top of high_tail_mult
        # Formula: weight = clip(1 + 2*|z|^1.5, 1, TAIL_WEIGHT_MAX)
        # Gives 2-3x more gradient to compounds >= 2 std from mean vs the linear |z| in v4_13d
        if TARGET_COL in df.columns:
            y_vals = df[TARGET_COL].values.astype(np.float64)
            y_mean = np.nanmean(y_vals)
            y_std  = np.nanstd(y_vals) + 1e-8
            z_scores = np.abs(y_vals - y_mean) / y_std
            z_tail_w = np.clip(1.0 + 2.0 * (z_scores ** 1.5), 1.0, TAIL_WEIGHT_MAX)
            n_ztail = (z_scores > 1).sum()
            if sw is None:
                sw = z_tail_w
            else:
                sw = np.clip(sw * z_tail_w, 0.1, TAIL_WEIGHT_MAX)
                sw = sw / sw.mean()
            print(f"v4_13f z-tail upwt : {n_ztail} compounds |z|>1  "
                  f"weight=[{z_tail_w.min():.2f}, {z_tail_w.max():.2f}]  "
                  f"combined_sw=[{sw.min():.2f},{sw.max():.2f}]  (cap={TAIL_WEIGHT_MAX}x)")
